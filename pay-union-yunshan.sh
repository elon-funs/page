...
function createOrder(){
	order
    step=0;
    if [[ uiexists "收款码"` == 1 ]];then
        step=1
        uiexists "收款码" .click()
    fi;
    if [[ uiexists "清除金额"` == 1 ]];then
        step=2
        uiexists "清除金额" .click()
    fi;
    if [[ uiexists "设置金额"` == 1 ]];then
        step=3
        uiexists "设置金额" .click()
    fi;
    if [[ step -eq 0 ]];then
...
while [[ 1 ]]; do
    ...
    if [[ ${orders} != '0' ]]; then
        ...
        for order in ${orderlist[@]}; do
            orderinfo=`getorderifo`
            haspush=`check "${orderinfo}"`; wait;
            if [[ "${phone}" == "${haspush}" ]]; then                
                echo "already ...";
            fi;
            sleep 0.5;
        done;
...
function createOrder(){
	step=0;
	if [[ poco(text="收款码").exists()==True ]]; then
		step=1
		poco(text="收款码").click()
	if [[ poco(text="清除金额").exists()==True ]]; then
		step=2
		poco(text="清除金额").click()
	if [[ poco(text="设置金额").exists()==True ]]; then
		step=3
	...
	for i in range(1,180 ]]; then
		if paid==False:
			...
			if paid==True:
				...
				if(amount.replace('\xa5','')==order['amount'] ]]; then
